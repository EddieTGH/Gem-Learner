				...
				<div className="rag-chat-window">
					{promptResponses.length === 0 && (
						<div className="greeting-container">
							<h1 className="greeting-text">Welcome to Career Prep by Pitt!</h1>
							<p className="greeting-subtext">This is a RAG Model developed using the Pitt CSC's public wiki</p>
						</div>
					)}
					{promptResponses.map((response, index) => (
						<div
							key={index}
							className={`rag-response-text ${response.isUser ? 'rag-user-query' : 'rag-bot-response'
								}`}
						>
							{response.text}
						</div>
